            ...
            {range(numOfGroups).map((groupIndex) => (
              <div key={groupIndex} className={styles.group}>
                {range(numOfItemsPerGroup).map((index) => {
                  return (
                    <motion.div
                      layoutId={`${widgetId}-${index + groupIndex * numOfItemsPerGroup}`}
                      key={`${widgetId}-${index + groupIndex * numOfItemsPerGroup}`}
        ...
        {includeRemainderArea && (
          <div className={styles.remainderArea}>
            ...
            {range(remainder).map((index) => {
              return (
                <motion.div layoutId={`${widgetId}-${index + numOfItems - remainder}`}
                  key={`${widgetId}-${index + numOfItems - remainder}`} className={styles.item} />
              );
            })}
          </div>
        )}    
